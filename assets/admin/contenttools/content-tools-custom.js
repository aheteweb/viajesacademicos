...
(function() {
  ...
  ContentEdit.Image = (function(_super) {
    ...
    Image.prototype.mount = function() {
    	...
    	this._domElement.setAttribute('class', 'img-fluid');
...
(function() {
  ...
  ContentTools.Tools.Image = (function(_super) {
    ...
    Image.apply = function(element, selection, callback) {
      ...
      $('#imagesModal .use').addClass('content-tools');
			_ref = this._insertAt(element), node = _ref[0], index = _ref[1];
			$(document).one('click', '#imagesModal .content-tools', function(){
				//Define the image element to insert
					imageAttrs = {};
			    imageAttrs.src = selectedImages[0];
			    image = new ContentEdit.Image(imageAttrs);
				//Inserts the image at the end of the paragraph
		      node.parent().attach(image, index);
		      image.focus();
		    $('#imagesModal .use').removeClass('content-toolsyy');
		    editor.stop('true');
		    editor.start();
				$('#imagesModal').modal('toggle');
			});      
      /*
	      var shit = '<img src="https://picsum.photos/200/300" class="ce-element ce-element--type-image ce-element--focused">'
				_ref = this._insertAt(element);
	      var originalContent = _ref[0]._domElement.innerHTML;
	      var range = _ref[0]._savedSelection
	      var originalStart = range._from;
			  var originalEnd = range._to;
			  var front = originalContent.substring(0, originalStart);
			  var back = originalContent.substring(originalEnd, originalContent.length);
			  _ref[0]._domElement.innerHTML = front + shit + back;
			*/

      
			
			/*Inserts the image at the beginning of the paragraph
				parent = element.parent();
		    insertAt = parent.children.indexOf(element);
		    parent.attach(image, insertAt);			
			*/
